...
<head>
  <meta charset="utf-8">
  <title>Auto-Plac | Automobili</title>
              ...
              <div class="col-md-9 col-md-push-3">
                <div class="result-sorting-wrapper">
                  <div class="sorting-count">
                    ...
                    $sql = "SELECT id from automobili";
                    $query = $dbh -> prepare($sql);
                    $query->execute();
                    $results=$query->fetchAll(PDO::FETCH_OBJ);
                    $cnt=$query->rowCount();
                    ?>
                    <p><span><?php echo htmlentities($cnt);?> Broj Automobila</span></p>
                  </div>
                </div>

                <?php $sql = "SELECT automobili.*,marka.Brand,marka.id as bid  from automobili join marka on marka.id=automobili.Marka order by rand() ";
                $query = $dbh -> prepare($sql);
                $query->execute();
                $results=$query->fetchAll(PDO::FETCH_OBJ);
                if($query->rowCount() > 0)
                {
                  ...
                  {  
                    ...
                    <div class="product-listing-m gray-bg">
                      <div class="product-listing-img"><img src="img/vehicleimages/<?php echo htmlentities($result->Vimage1);?>" class="img-responsive" alt="Image" style="height: 150px; width:300px;" /> </a> 
                      </div>
              ...
              <aside class="col-md-3 col-md-pull-9">
                <div class="sidebar_widget">
                  <div class="widget_heading">
                    <h5><i class="fa fa-car" aria-hidden="true"></i> Automobili za prodaju</h5>
                  </div>
                  <div class="recent_addedcars">
                    <ul>
                      <?php $sql = "SELECT automobili.*,marka.Brand,marka.id as bid  from automobili join marka on marka.id=automobili.Marka order by automobili.id desc limit 4";
                      $query = $dbh -> prepare($sql);
                      $query->execute();
                      $results=$query->fetchAll(PDO::FETCH_OBJ);
                      $cnt=1;
